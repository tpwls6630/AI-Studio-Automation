...
def open_browser(user_data_dir: str,
    ...
    subprocess.run(f'"{chrome_path}" --remote-debugging-port=9222 --user-data-dir="{user_data_dir}"')
...
def get_playwright():
    try:
        ...
        raise Exception("Error occured while getting playwright: ", str(e))
    finally:
        p.stop()

def connect_to_browser(p: Playwright) -> Browser:
    try:
        browser = p.chromium.connect_over_cdp("http://localhost:9222/")
    except Exception as e:
        raise Exception("Error occured while connecting to the browser: ", str(e))
...
def main():
    ...
    # Playwright 초기화
    p = next(get_playwright())

    # CDP 브라우저와 연결
    browser = connect_to_browser(p)
